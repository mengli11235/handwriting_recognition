...
class Network:
    def __init__(self):
        ...
        self.num_convolutions = 2
        self.division_factor = self.num_convolutions*self.num_convolutions
        ...
        self.cnn_fc = (self.input_size[0]/self.division_factor) * (self.input_size[0]/self.division_factor) * self.nf_2
        #Fully connected
        self.fc_1 = 1024
        self.fc_2 = 1024
    ...
    def build(self):
        ...
        h_pool2_flat = tf.reshape(h_pool2, [-1, int(self.cnn_fc)])

        W_fc1 = weight_variable([int((self.input_size[0] / self.division_factor) * (self.input_size[0] / self.division_factor) * self.nf_2), self.fc_1])
        ...
        l2 = 0.01 * (tf.nn.l2_loss(W_fc2) + tf.nn.l2_loss(W_fc1) + tf.nn.l2_loss(W_conv1) + tf.nn.l2_loss(W_conv2))
        loss = cross_entropy + l2
        CE = tf.nn.softmax_cross_entropy_with_logits_v2(labels=self.y_, logits=out)
        ...
        self.train_step = tf.train.AdamOptimizer(self.learning_rate).minimize(CE)
    ...
    def train_batch(self, batch_xs, batch_ys):
        ...
        lr,acc = self.sess.run([self.train_step, self.accuracy],
